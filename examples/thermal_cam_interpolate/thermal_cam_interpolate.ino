...
void loop() {
    ...
    uint16_t boxsize = min(320 / INTERPOLATED_COLS, 240 / INTERPOLATED_COLS);
    // uint16_t boxsize = 10;
...
void drawpixels(float* p, uint8_t rows, uint8_t cols, uint8_t boxWidth, uint8_t boxHeight) {
    ...
    for (int y = 0; y < rows; y++) {
        for (int x = 0; x < cols; x++) {
            ...
            uint16_t color;
            color = val * 2;
            Tft.fillRectangle(boxHeight * y, YStart + boxWidth * x, boxWidth, boxHeight, camColors[colorIndex]);
        }
    }
}
/*********************************************************************************************************
